...
def run_variant_call(msa_path: str, prefix: str|None=None, n_cpu: int=1) -> None:
    ...
    with open(msa_path, 'r') as msa_file:
        first_record = next(SeqIO.parse(msa_file, 'fasta'))
    first_seq_id = str(first_record.id)
    
    SeqIO.write(
        SeqRecord(Seq(msa.consensus), id=msa_path, description='consensus sequence'), 
        os.path.join(prefix, f'{first_seq_id}_consensus.fasta'), format='fasta'
    )

    out_path = os.path.join(prefix, f'{first_seq_id}_var.csv')
...
def run_preprocess(msa_path: str, prefix: str|None=None, n_cpu: int=1):
    ...
    var_path = run_variant_call(msa_path, prefix, n_cpu)

    # get fasta_path
    logger.info("Extracting the targeted genome from the MSA...")
    with open(msa_path, 'r') as msa_file:
        first_record = next(SeqIO.parse(msa_file, 'fasta'))
        targeted_genome = str(first_record.seq).replace('-', '').lower()
    
    targeted_genome_id = os.path.splitext(os.path.basename(msa_path))[0]
    first_seq_id = str(first_record.id)
    fasta_path = os.path.join(prefix, f"{first_seq_id}_targeted_genome.fasta")
    SeqIO.write(
        SeqRecord(Seq(targeted_genome), id=targeted_genome_id, description="Targeted genome without gaps"),
        fasta_path,
        "fasta"
    )
    logger.info(f"Targeted genome saved to {fasta_path}")

    return fasta_path, var_path
