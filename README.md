...
### Input files
Olivar has two input modes:
(1) A MSA file in FASTA format is required, BLAST database is optional. The first FASTA record is considered as the reference.
(2) A reference sequence in FASTA format is required, coordinates of sequence variations and BLAST database are optional. Only the first FASTA record is considered. 

 - (Required by mode 1) A MSA file that contains all sequences in FASTA format ([example](example_input/H1N1-HA.fasta)). The first record is considered as the reference sequence.
...
```
olivar sensitivity example_output/olivar-design.csv example_output/aligned_msa.fasta --pool 1  -o example_output -p 1
```
| Default name &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; | Description|
| :-------  | :-------- | 
